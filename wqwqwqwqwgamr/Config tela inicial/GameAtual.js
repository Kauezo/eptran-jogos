...
window.onload = function() {
    ...
    dino.style.backgroundImage = "url('../boneco1.png')"; // Define a imagem como fundo
    dino.style.backgroundSize = "cover"; // Ajusta para cobrir todo o elemento
    dino.style.backgroundPosition = "center"; // Centraliza a imagem
    dino.style.backgroundRepeat = "no-repeat"; // Evita repetição
};
...
function createCactus() {
    ...
    if (attempts < 10) {
        ...
        cactus.style.height = "185px";

        // Seleciona uma imagem aleatória de cactos
        ...
        cactus.style.backgroundSize = "cover"; // Ajusta para cobrir todo o elemento
        cactus.style.backgroundPosition = "center"; // Centraliza a imagem
        cactus.style.backgroundRepeat = "no-repeat"; // Evita repetição
...
function createPoint() {
    if (point || !gameIsActive) return; // Só cria nova moeda se o jogo estiver ativo

    point = document.createElement("div");
    point.classList.add("point");
    point.style.width = "60px";
    point.style.height = "60px";
    point.style.backgroundImage = "url('../moeda.png')"; // Define a imagem como fundo
    point.style.position = "absolute";
    point.style.top = "380px";
    point.style.backgroundSize = "cover";
    point.style.backgroundPosition = "center"; // Centraliza a imagem
    point.style.backgroundRepeat = "no-repeat";
    
    
    // Definindo uma posição aleatória limitada na horizontal (left)
    const minLeft = 100;
    const maxLeft = 700;
    ...
    pointInterval = setInterval(() => {
        ...
        if (
            ...
            score+=10; //Quantidade de valor da moeda
            scoreDisplay.innerText = `Score: ${score.toString().padStart(7, '0')}`;
            point.remove();
            point = null;
            clearInterval(pointInterval);
            
            // Cria nova moeda após 2 segundos, se o jogo ainda estiver ativo
            if (gameIsActive) {
